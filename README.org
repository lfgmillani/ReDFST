...
Compile with
#+BEGIN_SRC sh
gcc -o example example.c -fopenmp -lredfst -lcpupower -lm
#+END_SRC
or
#+BEGIN_SRC sh
gcc -o example example.c -fopenmp -lredfst -lcpufreq -lm
#+END_SRC
...
#+BEGIN_SRC sh
sudo setcap cap_sys_rawio=pei example
#+END_SRC
Run it:
#+BEGIN_SRC sh
./example
