...
#+BEGIN_SRC sh
export REDFST_2000000=1
export REDFST_1500000=2
export OMP_PROC_BIND=TRUE
./example2
#+END_SRC

The pairing (/frequency/, /region(s)/) is given by the environment
variable =REDFST_FREQUENCY=REGIONS=, where /FREQUENCY/ is the
frequency in KHz, and /REGIONS/ is a set. The set is in the format
region[,region...]  (example: 1,2,4-6 means the regions 1, 2, 4, 5 and
6). The use of =OMP_PROC_BIND= to bind the threads to cores is
obligatory.
...
- ~REDFST_FREQUENCY~ :: Defines a frequency/regions pairing. The
     regions are given as a comma-separated list. Example:
     REDFST_2000000=0,4-6 will execute regions 0, 4, 5 and 6 at 2GHz.
     The 63th region is entered when the program finishes. The other
